...
def create_socket(bind):
    try:
        ...
        socket.bind(bind)
        socket.listen(5)
        print("[DNS TS1]: TS1 socket created")
    except:
        print("[DNS TS1]: ERROR OCCURED")
    return socket
def running(client_socket,dns):
    while True:
        data = client_socket.recv(2048).decode("utf-8").strip()
        print("[DNS TS1]: Received Message: " +  str(data))
        if not data:
            return
        if data in dns:
            print("[DNS TS1]: Data found in DNS")
            return_data = dns.get(data) + ' IN'
